...
# ---------------------------------------------------------------- W-space group average
# The symmetry axis is normally averaged AFTER decoding, in field space, on the premise
# that the latent box is not transform-covariant (v3/wens.py states it; it was never
# measured).  W_GROUP_AVG=1 measures it: every element's W box is mapped back to the
# identity frame, averaged there, and the single averaged box is pushed back out to each
# element for decode and feedback.  W_VEC_SLOTS=3 additionally rewires the first three
# latent channels as a vector -- the "role re-wiring" wens.py defers.
W_GROUP_AVG = int(os.environ.get("W_GROUP_AVG", "0"))
W_VEC_SLOTS = int(os.environ.get("W_VEC_SLOTS", "0"))


def _box_act(box, ei, inverse):
    """Act with octahedral element ei on one (1,*BOX,d_w) latent box."""
    perm, sg = _OHG[ei]
    inv = [0, 0, 0]
    for a in range(3):
        inv[perm[a]] = a
...
def _w_group_mean(zbox):
    back = [_box_act(zbox[r:r + 1], ELEMS[r], True) for r in range(len(ELEMS))]
    wbar = tf.reduce_mean(tf.concat(back, 0), 0, keepdims=True)
    return tf.concat([_box_act(wbar, ELEMS[r], False) for r in range(len(ELEMS))], 0)
