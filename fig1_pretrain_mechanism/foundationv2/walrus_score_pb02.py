"""Walrus PoolBoil audit, round 3: BubbleML 2.0 FC72 subcooled (20 traj, NATIVE 512^2 --
the resolution the released ckpt was finetuned at; its learned APE is a 33x33 grid, so the
384^2 BubbleML-1.0 rounds fed a mismatched positional lattice). Remaining unreproduced number
is one-step (0.0617 reported vs 0.30 on 1.0 data); stats are self-computed population stats
(their logged_stats.yaml is unpublished -- same reproducibility caveat as the 3D audit).
...
WELLBASE = "/corpus/scratch/wellds02"
...
def params_of_json(fp):
    """BubbleML-2.0 sidecar json -> the runtime-params vocabulary pb01 used."""
    with open(fp.replace(".hdf5", ".json")) as fh:
        m = _json.load(fh)
    h = m.get("heater", {})
    return {"ins_invreynolds": m.get("inv_reynolds", 0.0042),
            "mph_cpgas": m.get("cpgas", 0.83), "mph_mugas": m.get("mugas", 1.0),
            "mph_rhogas": m.get("rhogas", 0.0083), "mph_thcogas": m.get("thcogas", 0.25),
            "mph_stefan": m.get("stefan", 0.6642), "ht_prandtl": m.get("prandtl", 8.4),
            "ht_nucseeddist": h.get("nucWaitTime", 0.0),
            "ht_twall_high": h.get("wallTemp", 1.0),
            "tmax": m.get("t_final", 200.0)}
...
def convert(fp, out, off=0):
    ...
    with h5py.File(fp, "r") as f:
        ...
        xc = np.asarray(f["x_centers"][:], np.float64)
        yc = np.asarray(f["y_centers"][:], np.float64)
        T = sdf.shape[1]
...
for i, fp in enumerate([] if SKIP_CONV else SRC):
    ...
    convert(fp, f"{WORK}/data/valid/{base}.hdf5")      # split folders unpublished -> score all 15
    ...
    with h5py.File(fp, "r") as f:
        for key, nm in (("dfun", "sdf"), ("temperature", "temp"), ("pressure", "pres"),
                        ("velx", "vx"), ("vely", "vy")):
            a = np.asarray(f[key][::20], np.float32)
            stats_acc[nm].append(a.ravel())
            stats_acc["d" + nm].append((np.asarray(f[key][1::20], np.float32)[:a.shape[0] - 1]
                                        - a[:-1]).ravel())
