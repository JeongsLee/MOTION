...
# ---- 2-IC (v0-channel) patch: Wave-Layer only. Adds the observed du/dt = u(t1)-u(t1-1) (zeros at t1=0,
# consistent with the zero-initial-velocity ICs) as a THIRD input channel. Labels stay [u],[c] -> the
# model/metric are unchanged apart from in_channels=3 (scOT re-inits embedding for new channel counts,
# its standard transfer mechanism). Restores the operator S(t,U0)=[u,v,c] that the Poseidon SM defines
# but the released benchmark omits.
if [ -n "$SCOT_2IC" ]; then
python - <<'P2IC'
...
open(p, "w").write(s)
print("SCOT_2IC PATCH OK (Layer: input_dim=3, v0 channel)")
P2IC
fi
...
[ -n "$SCOT_TIMEARGS" ] && TIMEARGS="$SCOT_TIMEARGS"   # explicit override (e.g. ACE all-lead 19x1)
...
T0="${SCOT_T0:-0}"
echo "=== POSEIDON from-IC full-traj: frame $T0 -> frame t, t=$((T0+1))..$FT, direct (ar_steps=1) ==="
for t in $(seq $((T0+1)) "$FT"); do
  python -u -m scOT.inference --mode eval --model_path "${BEST:-$CKDIR}" \
    --dataset "$DS" --data_path "$DATA" --file "$OUT/${FT_TASK}_N${N}_fromIC_t${t}.csv" \
    --ckpt_dir /tmp/ck2 --initial_time "$T0" --final_time "$t" --ar_steps 1 $EVALJV 2>&1 | tail -1
done
