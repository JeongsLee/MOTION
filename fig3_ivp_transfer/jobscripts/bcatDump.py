    ...
    snippet = '''
                if True:  # FULLTEST_RELNORM: our-formula running mean over the FULL test set
                    ...
                    if len(_c) >= 100 and type not in self._ft_saved:
                        ...
                        _np.savez(f"/eu/_bcat100_{type}.npz", pred=_P.astype("float32"), gt=_G.astype("float32"))
                        self._ft_saved.add(type); print(f"DUMP_SAVED {type} {_P.shape} -> /eu/_bcat100_{type}.npz", flush=True)
'''
