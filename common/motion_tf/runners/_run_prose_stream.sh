#!/bin/bash
# Streaming PROSE launcher — NO cache copy. bash _run_prose_stream.sh <config>
# The tf.data pipeline (cfg.stream=True) reads shards/.npy directly from the object-storage volume
# lazily, in the background, overlapped with GPU compute (prefetch). Per-step IO is tiny (~one batch),
# so the slow volume keeps up — instant startup, no ~40GB copy, no ~106GB in-RAM load, unbounded scale.
set -e
...
# stream directly from the volume cache — NO copy to local SSD
export PREBUILT_DIR="$CODE_VOL/data/prose/prebuilt"
export PROSE_DATA_DIR="$CODE_VOL/data/prose"
echo "streaming from volume: $PREBUILT_DIR (no local copy)"
...
echo "=== launching $CONFIG (STREAMING, no copy) ==="
set +e
