...
DUMP="${DUMP:-$EU_VOL/results}"; EXP="${EXP:-prose_fd_cache}"; CKDIR="$DUMP/$EXP/v1"; mkdir -p "$CKDIR"
...
echo "=== torchrun nproc=$NGPU | cache data | $EXTRA | ckpt->$CKDIR (save_periodic=1) $RELOAD ==="
set +e
torchrun --standalone --nproc_per_node=$NGPU main.py \
  dump_path="$DUMP" exp_name="$EXP" exp_id=v1 save_periodic=${SAVE_PERIODIC:-1} $RELOAD model=prose_2to1 use_wandb=0 symbol.symbol_input=1 \
  ...
  $EXTRA 2>&1 | tee "$CKDIR/train.log"      # tee full stdout (incl per-epoch eval) to /eu so the curve survives log truncation
