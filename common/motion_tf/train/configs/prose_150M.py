"""~150M scale-up — the PROSE-comparable budget. fp32 (SciML standard) made to fit at N_p=64 via
...
class Cfg:
    ...
    split_basis = False
    field_gate = False       # descriptor (given) gate — accuracy-first
    input_router = False
    lambda_gate = 1e-3
    ...
    jit_compile = False     # REQUIRED for swap_memory (XLA ignores it)
    swap_memory = True      # offload rollout activations to host RAM → fp32 N_p=64 at 150M fits
    save_dir = "results/prose_150M_test"
    ckpt_every = 500
    seed = 0
