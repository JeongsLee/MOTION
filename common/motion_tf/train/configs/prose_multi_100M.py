...
class Cfg:
    ...
    T_final = 1.0
    domain_L = 1.0
    N_p = 32                 # bf16 ALONE didn't fit N_p=64 at 92M (OOM 71GB temp); bf16+N_p32 fits
    ...
    steps = 6000              # frontier validation; extend after confirming NS improves at scale
    batch = 1                 # per-replica (global 8); raise per step-1 [mem] print
    lr = 3e-4
    lr_decay = True
    clipnorm = 1.0
    bf16 = True               # Section-1 (given-coeff, PROSE-FD arena) trains in bf16 like PROSE;
    ...
    # checkpointing
    save_dir = "results/prose_multi_100M"
    ckpt_every = 1000
    seed = 0
