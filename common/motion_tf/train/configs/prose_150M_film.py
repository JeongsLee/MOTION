"""150M-full with TRANSFORMER+FiLM router (head-to-head vs conv-router prose_150M_full, NS 21%).
Includes re-prepped incom_ns (340). 5 experts (same as baseline — isolates the router change).
...
class Cfg:
    ...
    field_gate = False
    input_router = False
    attn_router = True        # TRANSFORMER router (verified data->weights mapping) — vs conv-sigmoid baseline
    route_film = True         # FiLM(γ,β) route-context (domain+point) — reduces multi-family interference
    router_dim = 256
    router_depth = 2
    router_heads = 4
    router_patch = 2          # latent grid is 32² (LF=4) → patch2 → 16×16=256 tokens
    ...
    swap_memory = False
    latent_factor = 4       # experts at 32² (128/4) → ~16x less rollout activation → 150M fp32 fits
    save_dir = "results/prose_150M_film"
    ckpt_every = 1500
    seed = 0
