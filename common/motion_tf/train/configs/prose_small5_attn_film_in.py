...
class Cfg:
    ...
    route_context = False
    route_film = True            # FiLM(γ,β) route-context: domain(pooled)+point(pixel) (Poseidon/Unisolver)
    ...
    lr_decay = True
    warmup = 300
    instance_norm = True         # PROSE-style per-trajectory norm — TEST if it helps + matches PROSE metric space
    clipnorm = 1.0
    grad_accum = 4
    bf16 = False
    grad_checkpoint = False
    jit_compile = True
    save_dir = "results/prose_small5_attn_film_in"
    ckpt_every = 500
    seed = 0
