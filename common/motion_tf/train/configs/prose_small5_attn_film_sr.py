...
class Cfg:
    ...
    route_context = False
    route_film = True            # FiLM(γ,β) route-context (domain+point)
    step_embed = True            # ① panel-step sinusoidal embedding (NTO-ADA recursive)
    w_spatial = True             # ② spatial-context W readout (depthwise, before shared ADA basis)
    ...
    jit_compile = True
    save_dir = "results/prose_small5_attn_film_sr"
    ckpt_every = 500
    seed = 0
