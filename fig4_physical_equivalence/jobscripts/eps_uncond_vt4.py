...
def run_family(fi, nm):
    ...
    VT = 4 if nm.startswith("pdearena_uncond") else 10      # uncond: 14 real frames
    sc = std[sel][:, None, None, None, :] + 1e-6; mc = mean[sel][:, None, None, None, :]
    gt = xtg[sel][:, 1:1 + VT] * sc + mc
    def rel(p):
        pp = p[:, :VT] * sc + mc
    ...
    def _eps_of(Pg, P0):
        A = Pg.astype(np.float64)[:, :VT] * sc + mc; B = P0.astype(np.float64)[:, :VT] * sc + mc
