...
for TAGSPEC in id:0:0 fr:1:0 fc:0:1 r180:1:1; do
  ...
  FLIP_FR=$FR FLIP_FC=$FC FLIPTAG=$TAG torchrun --standalone --nnodes 1 --nproc_per_node "$NGPU" main.py \
    ...
    max_epoch=81 n_steps_per_epoch=2 batch_size=4 2>&1 | grep -E "FULLTEST|DUMP_SAVED|Error|Traceback" | tail -20
  set -e
done
python - <<'CEOF'
import numpy as np, glob, os
fams = sorted(set(os.path.basename(p).split("_", 2)[2][:-4] for p in glob.glob("/eu/_prosesym_id_*.npz")))
...
def inv(p, tag):
    ...
    return p
for fam in fams:
    d0 = np.load(f"/eu/_prosesym_id_{fam}.npz"); gt = d0["gt"]
    preds = {}
    for tag in ("id", "fr", "fc", "r180"):
        f = f"/eu/_prosesym_{tag}_{fam}.npz"
        if os.path.exists(f):
