...
def run_family(fi, nm):
    ...
    # equivariance defect in the SAME convention as rel(): physical units, active channels
    _m5 = (np.abs(std[sel]) > 0)[:, None, None, None, :].astype(np.float64)
    _P0 = preds_g[(0, 0, 0)].astype(np.float64) * sc + mc
