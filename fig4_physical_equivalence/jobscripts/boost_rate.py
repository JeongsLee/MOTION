...
for (p, q) in [(1, 4), (1, 2), (1, 1), (2, 1), (3, 1)]:
    ...
    # DECOMPOSITION: undo the coordinate shift at an assumed drift RATE alpha*V instead
    # of V.  alpha=1 is the true Galilean inverse; the minimising alpha is the rate the
    # operator actually transported at, and the defect there is what is left once a pure
    # rate deficit is accounted for.  A covariant operator minimises at alpha=1.
    best = (1e9, None)
    for a10 in range(0, 14):
        al = a10 / 10.0
        Q = np.array(noroll)
        for j in fr:
            Q[:, j] = np.roll(Q[:, j], -int(round(al * (p * j) / q)), axis=AX + 1)
        e = _rel_frames(Q, P0q, fr)
        if e < best[0]: best = (e, al)
    print(f"[{task}]   defect vs assumed drift rate: min={best[0]:.3f}% at "
          f"alpha={best[1]:.1f} (alpha=1 is the true inverse)", flush=True)
