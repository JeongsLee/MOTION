...
def tg(x, g, inv=False):
    ...
    if t == 'sh':
        s = 1 if inv else -1
        return torch.roll(x, shifts=(s * fr, s * fc), dims=(-2, -1))
    def _fl(x):
        if fr:
...
for t in times:
    ...
    for g in GROUP:
        ...
        o = rollout(trainer, ts, ar_steps=max(1, t // 2) if int(os.environ.get('AR_MODE', '0')) else 1)
...
print(f"[scot {ds}] EPS_EQUIV%s(%%, task metric):" % ('_AR' if int(os.environ.get('AR_MODE','0')) else ''),
