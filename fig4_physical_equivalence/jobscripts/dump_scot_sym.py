...
def tg(x, g):
    ...
    if fr:
        ...
        if VEL: x[:, 0] = -x[:, 0]                      # ch0 pairs with ROW axis (verified layout)
    if fc:
...
for arr, tag in [(per_g[(0,0)], "single"), (ens, "ENSEMBLE")]:
...
np.savez_compressed(out, pred16=ens[:16].astype(np.float32), ref16=R[:16].astype(np.float32))
