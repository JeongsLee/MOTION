    ...
    def __iter__(self):
        ...
        for k in rng:
            ...
            if _fr:
                ...
                if data.shape[-1] >= 2 and int(os.environ.get("FLIP_SIGN","1")): data[..., 0] = -data[..., 0]
            if _fc:
                data = np.flip(data, axis=2).copy()
                if data.shape[-1] >= 2 and int(os.environ.get("FLIP_SIGN","1")): data[..., 1] = -data[..., 1]
...
NGPU=$(python -c "import torch;print(torch.cuda.device_count())")
for TAGSPEC in frns:1:0; do
  TAG="${TAGSPEC%%:*}"; REST="${TAGSPEC#*:}"; FR="${REST%%:*}"; FC="${REST##*:}"
  echo "=== FLIP $TAG (FR=$FR FC=$FC) ==="
  set +e
  FLIP_FR=$FR FLIP_FC=$FC FLIP_SIGN=0 FLIPTAG=$TAG torchrun --standalone --nnodes 1 --nproc_per_node "$NGPU" main.py \
...
python - <<'CEOF'
import numpy as np
d0 = np.load("/eu/_prosesymE_id_cfdbench.npz"); gt = d0["gt"]; pid = d0["pred"]
pf = np.load("/eu/_prosesymE_frns_cfdbench.npz")["pred"]
pf = np.flip(pf, axis=2).copy()          # inverse row-mirror, NO sign (u-like scalar channels)
def rel(p):
    num = np.sqrt(((p - gt) ** 2).sum((2, 3, 4))); den = np.sqrt((gt ** 2).sum((2, 3, 4))) + 1e-12
    return 100 * (num / den).mean()
ens = (pid + pf) / 2
print(f"[PROSE cfdbench] id={rel(pid):.3f} frns={rel(pf):.3f} K2-ENS={rel(ens):.3f}", flush=True)
