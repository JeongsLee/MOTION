...
python - <<'CEOF'
import numpy as np
d0 = np.load("/eu/_bcatsymE_id_cfdbench.npz"); gt = d0["gt"]; pid = d0["pred"]
pf = np.load("/eu/_bcatsymE_frns_cfdbench.npz")["pred"]
...
ens = (pid + pf) / 2
print(f"[BCAT cfdbench] id={rel(pid):.3f} frns={rel(pf):.3f} K2-ENS={rel(ens):.3f}", flush=True)
