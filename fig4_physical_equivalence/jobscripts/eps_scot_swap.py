...
V0 = int(os.environ.get("VEL0", "0"))   # index of the first velocity channel
SWP = int(os.environ.get("VEL_SWAP", "0"))  # pair the row axis with V0+1 instead
...
def tg(x, g, inv=False):
    ...
    def _fl(x):
        if fr:
            ...
            if VEL: x[:, V0 + (1 if SWP else 0)] = -x[:, V0 + (1 if SWP else 0)]
        if fc:
            x = torch.flip(x, dims=[-1]).clone()
            if VEL: x[:, V0 + (0 if SWP else 1)] = -x[:, V0 + (0 if SWP else 1)]
        return x
    def _tr(x):
        if t:
            x = x.transpose(-2, -1).contiguous()
            if VEL:
                _o = list(range(x.shape[1])); _o[V0], _o[V0 + 1] = V0 + 1, V0
                x = x[:, _o].clone()
        return x
