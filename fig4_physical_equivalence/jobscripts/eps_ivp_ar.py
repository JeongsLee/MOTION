...
if do_ar:
    ...
    # AR-mode defect: same rel-L1 convention as armet(), scored on the AR targets
    _msA = np.zeros_like(cm)
    for _c in _SOL.get(task, list(range(cm.shape[-1]))):
        _msA[:, _c] = cm[:, _c]
    _m5A = _msA[:, None, None, None, :]
    def _epsA(Pg, P0):
        A = Pg * sc + mc; Bb = P0 * sc + mc
        num = np.sum(np.abs(A - Bb) * _m5A, axis=(2, 3, 4))
        den = np.sum(np.abs(Bb) * _m5A, axis=(2, 3, 4)) + 1e-8
        return float(np.median((num / den).mean(axis=1)) * 100)
    _lbA = {(0,1,0):"fr",(0,0,1):"fc",(0,1,1):"r180",(1,0,0):"d1",(1,1,0):"r90",(1,0,1):"r270",(1,1,1):"d2"}
    _P0A = ar_g[(0, 0, 0)]
    _epsAR = {_lbA[g]: _epsA(ar_g[g], _P0A) for g in GROUP if g != (0, 0, 0)}
    print(f"[{task}] EPS_EQUIV_AR(%):", {k: round(v, 3) for k, v in sorted(_epsAR.items(), key=lambda kv: kv[1])}, flush=True)
