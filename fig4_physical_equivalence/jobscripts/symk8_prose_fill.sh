...
NGPU=$(python -c "import torch;print(torch.cuda.device_count())")
for TAGSPEC in id:0:0:0 fr:0:1:0; do
...
for fam in os.environ.get("FAMS", "").split(","):
    ...
    pid = d0["pred"]
    pfr = inv(np.load(f"/eu/_prosesymE_fr_{fam}.npz")["pred"], GD["fr"])
    def rel(a, b):
        num = np.sqrt(((a - b) ** 2).sum((2, 3, 4)))
        den = np.sqrt((b ** 2).sum((2, 3, 4))) + 1e-12
        return 100 * float((num / den).mean())
    print(f"[PROSE {fam}] err_id={rel(pid, gt):.3f}%  err_fr={rel(pfr, gt):.3f}%  "
          f"EPS_fr={rel(pfr, pid):.3f}%", flush=True)
