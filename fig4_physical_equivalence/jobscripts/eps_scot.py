...
P0 = per_g[(0, 0, 0)].astype(np.float64)
_den = np.sqrt((P0 ** 2).sum(axis=(2, 3)))
_lb = {(0,1,0):"fr",(0,0,1):"fc",(0,1,1):"r180",(1,0,0):"d1",(1,1,0):"r90",(1,0,1):"r270",(1,1,1):"d2"}
_eps = {}
for g in GROUP:
    if g == (0, 0, 0): continue
    _num = np.sqrt(((per_g[g].astype(np.float64) - P0) ** 2).sum(axis=(2, 3)))
    _eps[_lb[g]] = float(np.median((_num / (_den + 1e-12)).ravel()) * 100)
print(f"[scot {ds}] EPS_EQUIV(%):", {k: round(v, 3) for k, v in sorted(_eps.items(), key=lambda kv: kv[1])}, flush=True)
