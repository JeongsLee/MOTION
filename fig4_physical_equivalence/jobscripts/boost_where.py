...
print(f"[{task}] {'V/u_rms':>8} {'cells/fr':>9} {'frames':>7} {'err_id':>8} "
      f"{'err_boost':>10} {'eps':>8} {'eps_noroll':>11} {'eps_wrongsgn':>12}", flush=True)
for (p, q) in [(1, 4), (1, 2), (1, 1), (2, 1), (3, 1)]:
    ...
    if not fr:
        continue
    # forward boost is  u'(x,t) = u(x - Vt, t) + V, i.e. frame t rolled by +V*t cells
    # (towards increasing index), so the INVERSE rolls by -V*t and subtracts V.
    inv = np.array(Pb); inv[..., SLOT] -= Vn[:, None, None, None]
    noroll = np.array(inv)
    wrong = np.array(inv)
    for j in fr:
        inv[:, j] = np.roll(inv[:, j], -((p * j) // q), axis=AX + 1)
        wrong[:, j] = np.roll(wrong[:, j], +((p * j) // q), axis=AX + 1)
    e_id = _rel_frames(P0q, xt[:, :NT], fr)
    e_bo = _rel_frames(inv, xt[:, :NT], fr)
    eps = _rel_frames(inv, P0q, fr)
    eps0 = _rel_frames(noroll, P0q, fr)
    eps_w = _rel_frames(wrong, P0q, fr)
    print(f"[{task}] {V/_urms:8.2f} {p/q:9.2f} {len(fr):7d} {e_id:8.3f} "
          f"{e_bo:10.3f} {eps:8.3f} {eps0:11.3f} {eps_w:11.3f}", flush=True)
    ...
    # ---- WHERE is the drift lost?  local deficit d(x) = <u_boost - u_id>(x); a fully
    # carried drift gives d = V everywhere.  A seam/connectivity loss concentrates the
    # deficit near the domain edge or at patch boundaries; a learned attenuation of the
    # k=0 mode is spatially uniform.
    loc = ((Pb[:, fr][..., SLOT] - P0q[:, fr][..., SLOT]) * _su).mean(axis=(0, 1))   # (H,W)
    carried_map = loc / V
    Hh, Ww = carried_map.shape
    ii = np.minimum(np.arange(Hh), Hh - 1 - np.arange(Hh))[:, None]
    jj = np.minimum(np.arange(Ww), Ww - 1 - np.arange(Ww))[None, :]
    dist = np.minimum(np.broadcast_to(ii, (Hh, Ww)), np.broadcast_to(jj, (Hh, Ww)))
    bands = [(0, 2), (2, 4), (4, 8), (8, 16), (16, 32), (32, 64)]
    print("[{}]   carried vs distance from domain edge (cells): ".format(task) +
          " ".join(f"{a}-{b}:{100*carried_map[(dist>=a)&(dist<b)].mean():.0f}%"
                   for a, b in bands), flush=True)
    print("[{}]   carried vs position within the 8-cell patch (col): ".format(task) +
          " ".join(f"{k}:{100*carried_map[:, k::8].mean():.0f}%" for k in range(8)), flush=True)
    per_frame = ((Pb[:, fr][..., SLOT] - P0q[:, fr][..., SLOT]) * _su).mean(axis=(0, 2, 3)) / V
    print("[{}]   carried vs lead time: ".format(task) +
          " ".join(f"{fr[k]}:{100*per_frame[k]:.0f}%" for k in range(0, len(fr), max(1, len(fr)//6))),
          flush=True)
