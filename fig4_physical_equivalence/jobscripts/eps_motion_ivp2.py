...
for g in GROUP:
    ...
    preds_g[g] = p
    metrics(p, f"g={g}")
# scored solution slots only (a fed-but-unsupervised channel like the Poisson source
# carries arbitrary output and must not enter an equivalence measurement)
_SOL = {"NS-PwC": [0, 1, 2], "ACE": [2], "Wave-Layer": [2], "Poisson-Gauss": [2]}
_ms = np.zeros_like(cm)
for _c in _SOL.get(task, list(range(cm.shape[-1]))):
    _ms[:, _c] = cm[:, _c]
_m5 = _ms[:, None, None, None, :].astype(np.float64)
...
_GT = xt[:, 1:1 + _P0.shape[1]] * _sc + _mc
_dg = np.sqrt((((_GT) ** 2) * _m5).sum(axis=(2, 3, 4))) + 1e-12
def _err(P):
    return float(np.median((np.sqrt((((P - _GT) ** 2) * _m5).sum(axis=(2, 3, 4))) / _dg).ravel()) * 100)
_e0 = _err(_P0)
print(f"[{task}] EPS_EQUIV(%):", {k: round(v, 3) for k, v in sorted(_eps.items(), key=lambda kv: kv[1])},
      f"| err_id={_e0:.3f}% bound(fr)={_e0 + _err(preds_g[(0,1,0)].astype(np.float64) * _sc + _mc):.3f}%", flush=True)
