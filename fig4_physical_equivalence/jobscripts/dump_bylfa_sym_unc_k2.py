...
def run_family(fi, nm, pair):
    ...
    VT = 4 if nm.startswith("pdearena_uncond") else 10       # uncond: 14 real frames -> 4 valid forecasts
    ...
    base = r[(0, 0)]
    legal = [(0, 0), (1, 0)]                       # FORCED K2: full legal group incl. degraded replica
...
fi = list(names).index("pdearena_uncond")
run_family(fi, "pdearena_uncond", 0)
