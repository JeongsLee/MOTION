...
_CAP = int(os.environ.get("LEAD_CAP", "0"))
if _CAP:
    NT = min(NT, _CAP + 1)          # score leads 1.._CAP only (closed-wave protocol)
print("test", xt.shape, "NT(scored)", NT, "lead_cap", _CAP, flush=True)
...
def metrics(pred, tag):
    ...
    pT = pred[:, 1:NT]*sc + mc; rT = xt[:, 1:NT]*sc + mc
...
def _eps_of(Pg, P0):
    A = Pg[:, 1:NT] * _sc + _mc; B = P0[:, 1:NT] * _sc + _mc
