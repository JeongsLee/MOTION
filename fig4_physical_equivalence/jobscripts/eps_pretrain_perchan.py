...
SLOT = {0: "vx", 1: "vy", 2: "tracer", 3: "density", 4: "pressure", 5: "energy",
        6: "geom", 7: "forcing"}

def _rel_ch(P, R, sel, c):
    """The same rel-L1 convention restricted to one channel of one family."""
    sc = std[sel][:, None, None, None, c]; mc = mean[sel][:, None, None, None, c]
    A = P[sel][:, 1:, :, :, c] * sc + mc; B = R[sel][:, 1:, :, :, c] * sc + mc
    num = np.sum(np.abs(A - B), axis=(2, 3)); den = np.sum(np.abs(B), axis=(2, 3)) + 1e-8
    return float(np.median((num / den).mean(axis=1)) * 100)

P0 = preds_g[(0, 0, 0)]
MIRR = [(0,1,0), (0,0,1), (0,1,1)]
ROT  = [(1,0,0), (1,1,0), (1,0,1), (1,1,1)]
for k in sorted(set(FAM.tolist())):
    nm = FNAMES[k] if k < len(FNAMES) else str(k)
    sel = np.where(FAM == k)[0]
    act = [c for c in range(cm.shape[-1]) if float(cm[sel][:, c].max()) > 0]
    print(f"[{nm}] n={len(sel)}  active channels: {[SLOT[c] for c in act]}", flush=True)
    for c in act:
        err = _rel_ch(P0, xt[:, :NT], sel, c)
        em = max(_rel_ch(preds_g[g], P0, sel, c) for g in MIRR)
        er = max(_rel_ch(preds_g[g], P0, sel, c) for g in ROT)
        print(f"    {SLOT[c]:9s} err={err:8.3f}%   eps_mirror={em:8.3f}% ({em/max(err,1e-9):6.2f}x)"
              f"   eps_rot={er:8.3f}% ({er/max(err,1e-9):6.2f}x)", flush=True)
print("PERCHAN_DONE", flush=True)
