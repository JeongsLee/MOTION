...
fams = sorted(set(os.path.basename(p).split("_", 2)[2][:-4] for p in glob.glob("/eu/_bcatsym_id_*.npz")))
...
def inv(p, tag):
    ...
    return p
for fam in fams:
    d0 = np.load(f"/eu/_bcatsym_id_{fam}.npz"); gt = d0["gt"]
    preds = {}
    for tag in ("id", "fr", "fc", "r180"):
        f = f"/eu/_bcatsym_{tag}_{fam}.npz"
        if os.path.exists(f):
