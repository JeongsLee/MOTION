...
def _inv_boost(P, sgn):
    Q = np.array(P)
    Q[..., SLOT] = Q[..., SLOT] - _Vn[:, None, None, None]
    for j in range(Q.shape[1]):
        Q[:, j] = np.roll(Q[:, j], sgn * VS * j, axis=AX + 1)
    return Q
P0q = preds_g[(0, 0, 0)]
...
print(f"[{task}] err_id={_relq(Bq, GTq):.3f}%", flush=True)
_mu_pred_b = float((Pb[:, 1:, ..., SLOT] * std[:, SLOT].reshape(-1,1,1,1) + mean[:, SLOT].reshape(-1,1,1,1)).mean())
_mu_pred_0 = float((P0q[:, 1:, ..., SLOT] * std[:, SLOT].reshape(-1,1,1,1) + mean[:, SLOT].reshape(-1,1,1,1)).mean())
_mu_gt = float((xt[:, 1:NT, ..., SLOT] * std[:, SLOT].reshape(-1,1,1,1) + mean[:, SLOT].reshape(-1,1,1,1)).mean())
print(f"[{task}] mean velocity along boost axis: GT={_mu_gt:+.4f} pred(unboosted)={_mu_pred_0:+.4f} "
      f"pred(boosted)={_mu_pred_b:+.4f} expected(boosted)={_mu_gt + _V:+.4f}", flush=True)
for sgn in (+1, -1, 0):
    A = _inv_boost(Pb, sgn)[:, 1:] * _scq + _mcq
    print(f"[{task}] BOOST(roll {sgn:+d}: +1=correct inverse, 0=model ignored the drift) eps={_relq(A, Bq):.3f}%  "
