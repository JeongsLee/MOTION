...
NGPU=$(python -c "import torch;print(torch.cuda.device_count())")
for TAGSPEC in d1:1:0:0 r90:1:1:0 r270:1:0:1 d2:1:1:1; do
  ...
  FLIP_T=$T FLIP_FR=$FR FLIP_FC=$FC FLIPTAG=$TAG torchrun --standalone --nnodes 1 --nproc_per_node "$NGPU" main.py \
    ...
    "data.types=[shallow_water,com_ns,incom_ns]" \
...
def inv(p, g):
    ...
    if t:
        ...
        if C >= 2:
            ...
            p = p[..., sw]
    return p
for fam in ("shallow_water", "com_ns", "incom_ns"):
    d0 = np.load(f"/eu/_prosesymE_id_{fam}.npz"); gt = d0["gt"]
    preds = {"id": d0["pred"]}
    for tag in ("fr", "fc", "r180", "d1", "r90", "r270", "d2"):
        preds[tag] = inv(np.load(f"/eu/_prosesymE_{tag}_{fam}.npz")["pred"], GD[tag])
    def rel(p):
        num = np.sqrt(((p - gt) ** 2).sum((2, 3, 4))); den = np.sqrt((gt ** 2).sum((2, 3, 4))) + 1e-12
        return 100 * (num / den).mean()
    r = {t: rel(p) for t, p in preds.items()}
    print(f"[PROSE {fam}] per-g:", {t: round(float(v), 3) for t, v in r.items()}, flush=True)
    ens4 = np.mean([preds[t] for t in ("id", "fr", "fc", "r180")], axis=0)
    ens8 = np.mean(list(preds.values()), axis=0)
    print(f"[PROSE {fam}] single={r['id']:.3f} K4-ENS={rel(ens4):.3f} K8-ENS={rel(ens8):.3f}", flush=True)
